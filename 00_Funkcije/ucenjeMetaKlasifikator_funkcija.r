...
ucenjeMetaKlasifikator <- function(imenaMnozic = c('trainBL','sosedSet', 'metaSet'), ## ime mnozice: [sosede katerih iscemo, iz katere so sosedje]
									...
									## nauci meta klasifikatorje za razlicne parametre
									{

	stHypParOpt <- length(K)*length(Kp)*length(knnALG)*length(baseLearner)*length(metaALG)*length(hC)
	zeIrac <- 0
	for(nSosedi in K){
			for(OPnSosedi in Kp){
				for(alg in knnALG){
					for(bl in baseLearner){
						...
						for(meja in hC){
							...
							if(length(izborR) < 50){
								print(paste('Samo', length(izborR), 'primerov iz metaSet-a ima dovolj razlicne napovedi (hC)!!!'))
							}
							if(is.null(izborR)){
								stop('noben primer ne preseze praga razlicnosti napovedi hC!!')
							}
							FM <-  metaPodatki$metaFM[izborR,]
							response <- metaPodatki$metaClass[izborR]
							##################################################################
							### UCENJE META KLASIFIKATORJA ###################################
							for(algM in metaALG){
								imeMetaKlasifikator <- paste0('metaKlasifikator[BL]', bl, '[trainBL]', imenaMnozic[1], 
